...
- `--ref-audio-seconds <seconds>` for exact `--estimate` quotes. MiniMax H3
  Max bills reference audio as pooled tokens, so an audio-bearing estimate
  that cannot see the combined duration quotes short.
- MiniMax H3 Max now accepts `--ref`, `--ref-video` and `--ref-audio` for its
  new reference-to-video mode: up to 9 images, 3 videos and 3 audio clips, 12
  files total, each video/audio clip 2-15s and 15s combined per modality.
  Reference audio cannot be the only reference, and `--ar adaptive` is
  reference-mode only. Frame mode and reference mode are separate provider
  endpoints, so `--start-image`/`--end-image` alongside references is refused
  up front rather than by the API.

### Changed

- The bundled `videodraft` skill documents H3 Max reference mode and its
  pooled-token pricing: the first 4,096 reference tokens are free and each
  additional 1,000 costs 2 credits, where an image is `(width x height) / 1024`
  tokens, a reference video is 2,886 tokens per second at 480p or 7,459 at
  768p, and reference audio is about 2,121 per second. The server measures
  each reference image, so a quote assuming 1024x1024 is a floor.
- An `--estimate` for H3 Max with reference media but no `--ref-video-seconds`
  / `--ref-audio-seconds` now returns `lower_bound: true` and names the flag
  needed for an exact quote, instead of silently omitting the surcharge.
- `--safety-checker` is documented as off by default. VideoDraft does not
  surface a provider safety checker on any model, so it is only sent when a
...
- Corrected the bundled `videodraft` skill's `edit video` entry, which still
  offered "up to 3 creative reference videos of at most 3s each" on an edit.
  An edit accepts exactly one input video, so `--ref-video` is refused there;
  the skill now says so and points at `generate video --video-task generate`
  for guiding a new clip with video references. 0.19.3 corrected this rule for
  `generate video` but missed the `edit video` paragraph.

## [0.19.3] - 2026-08-30

### Fixed

- `generate video` and `edit video` now reject a creative `--ref-video`
  alongside `--source-video` or `--previous-interaction-id` on Gemini Omni
  1.1 Flash. Google accepts exactly one input video for edit and extend and
  refuses the combination, so the CLI had been forwarding a request that could
  only fail. Reference videos remain supported on `--video-task generate`.

### Changed

- Corrected the bundled `videodraft` skill's Gemini Omni 1.1 Flash extension
  rules. It claimed uploaded-video extensions could not add new dialogue while
  previous-interaction extensions could; both halves were wrong. New dialogue
  is allowed only when the SOURCE video is silent, regardless of how the
  extension was requested. The skill also advertised reference videos
  alongside a source, and both of its extension examples were guaranteed to
  fail. The 40s ceiling is now stated with its real constraint: reachable only
  by extending from a source of 30s or less.
...
- Session-store concurrency hardening from adversarial review: the entire
  save (including the first-time claim) runs under the per-scope lock; lock
  release is ownership-verified; stale-lock takeover is atomic; contended
  reads never race an active writer; a read-only config dir degrades to the
  shared stateless fallback (or keeps a readable token stable) instead of
  minting a session per command; `sessions reset` reports a declined reset;
  `sessions current` paginates the session lookup and re-describes after
  the verification RPC.
- Connection-session scopes are partitioned by authentication identity:
  bare `--token` / `VIDEODRAFT_API_KEY` callers get a one-way token
  fingerprint instead of all sharing one "default" scope, so accounts
  alternating in the same directory no longer evict each other's session.

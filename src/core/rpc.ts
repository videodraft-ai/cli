    ...
    if (!this.handshake) {
      this.handshake = (async () => {
        ...
        try {
          ...
          stored = null; // a broken store must never break the command
        }
        if (stored) {
        ...
        try {
          ...
          if (response.ok && minted) {
            ...
            // concurrent process's that claimed the directory first — or
            // null when it could not be persisted (read-only config dir),
            // in which case we go stateless rather than creating a fresh
            // session on every short-lived invocation.
            this.sessionId = this.session!.save(minted);
            // Complete the lifecycle either way (we DID initialize); the
            // server answers 202, errors ignored. Stateless fallback sends

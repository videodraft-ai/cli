...
export function registerMediaCommands(program: Command): void {
  program
    .command("upload <file>")
    .description(
      "Upload a local image/video/audio file; prints the public CDN URL",
    )
    .option(
      "--content-type <mime>",
      "MIME override when the extension is ambiguous",
    )
    .action(async function (this: Command, file: string) {
      ...
      emit(ctx.out, result, (o) => {
        ...
        note(
          ...
          fmt.dim(
            o,
            "Use this URL anywhere a public media URL is accepted (--ref, --start-image, ...)",
          ),
        );
      });
    });
  ...
  program
    ...
    .description(
      "Describe an image with a vision model (local files are uploaded first)",
    )
    .action(async function (this: Command, source: string) {
      ...
      const result: any = await ctx.client.callTool("describe_image", {
        image_url: url,
      });
      emit(ctx.out, result, (o) => {
        const text =
          result?.description ?? result?.text ?? JSON.stringify(result);
  ...
  program
    ...
    .option(
      "-o, --output <path>",
      "output file or directory (default: basename in cwd)",
    )
    .action(async function (this: Command, url: string) {
      ...
      emit(ctx.out, file, (o) => {
        ...
        if (file.preview)
          note(o, fmt.dim(o, `inspect via preview: ${file.preview}`));
      });
    });
}

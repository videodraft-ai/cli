...
export function registerAccountCommands(program: Command): void {
  ...
  sessions
    ...
    .action(async function (this: Command) {
      ...
      // adopt a server re-minted token (secret rotation, account switch),
      // which the client persists via replace(). Describing the store before
      // the call would report — and even link — the superseded session id.
      const knownIds = new Set<string>();
      let listed = false;
      if (ctx.session) {
        // Paginate: absence from the first page is not proof the row does
        // not exist for accounts with many sessions. Bounded sweep; if the
        // account has even more, the conservative outcome is a suppressed
        // URL, never a broken one.
        try {
          for (let offset = 0; offset < 2000; ) {
            const page: any = await ctx.client.callTool(
              "list_ai_studio_sessions",
              { limit: 200, offset },
            );
            const pageRows: any[] = page?.sessions ?? [];
          ...
          listed = false; // offline/unknown → do not advertise the URL
        }
      }
  ...
  sessions
    ...
    .action(async function (this: Command) {
      ...
      if (opts.all) {
        ...
        // A declined reset (scope lock unavailable / read-only dir) leaves
        // the record in place — reporting success would let the next
        // invocation silently reuse the "forgotten" token.
        removed = had && ran ? 1 : 0;
        declined = had && !ran;
      }
      const result = {
      ...
      emit(ctx.out, result, () => {
        process.stdout.write(
          removed > 0
            ...
            : declined
              ? "Could not reset: the session store is locked or unwritable; the session may still be in use.\n"
              : "Nothing to reset.\n",
        );
      });
    });

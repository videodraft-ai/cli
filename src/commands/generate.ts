...
function estimateVideoModel(
  ...
  const wan3Task =
    ...
    Boolean(opts.webUrl);
  const h3MaxTask =
    opts.promptExpansionMode !== undefined || opts.safetyChecker !== undefined;
...
async function printEstimate(
  ...
  emit(ctx.out, {
    estimate,
    note: params.lowerBoundReason
      ? `No credits were spent (--estimate). LOWER BOUND: ${params.lowerBoundReason}`
      : "No credits were spent (--estimate).",
    ...(params.lowerBoundReason ? { lower_bound: true } : {}),
  });
}
    ...
    emit(
      ctx.out,
      {
        ...
        ...(result.payload?.continuation_available !== undefined
          ? {
              continuation_available: result.payload.continuation_available,
            }
          : {}),
...
export function registerGenerateCommands(program: Command): void {
  ...
  generate
    ...
    .option(
      "--ref-audio <url|file>",
      "reference audio (repeatable; Wan 3.0, MiniMax H3, MiniMax H3 Max, Seedance 2, or Seedance 2.5; local files uploaded)",
      collect,
      [],
    )
    .option(
    ...
    .option(
      ...
      "combined reference-video duration for an exact --estimate (Seedance 2.x bills input seconds, MiniMax H3 Max bills them as reference tokens; MiniMax H3 and Wan 3.0 do not). Max 30s on Seedance 2.5, 15s elsewhere",
    )
    .option(
      "--ref-audio-seconds <seconds>",
      "combined reference-audio duration for an exact --estimate. MiniMax H3 Max bills reference audio as pooled tokens; no other model bills it",
    )
    .option(
    ...
    .action(async function (this: Command, promptWords: string[] = []) {
      ...
      // H3 Max bills reference audio as pooled tokens, so an audio-bearing
      // estimate needs its combined duration to be exact.
      const refAudioSeconds = optionalRangedNumber(
        opts.refAudioSeconds,
        "--ref-audio-seconds",
        0,
        15,
      );
      ...
      if (
        ["h3-max", "h3max", "minimax_h3_max"].includes(String(opts.model || ""))
      ) {
        opts.model = "minimax-h3-max";
      }
      if (
      ...
      if (seed !== undefined && seed < 0 && opts.model !== "minimax-h3-max") {
        throw new CliError(
      ...
      if (hasGeminiOmniOnlyControls && opts.model !== "gemini-omni-1.1-flash") {
        throw new CliError(
      ...
      if (opts.model === "minimax-h3-max") {
        ...
        const audioCount = Array.isArray(opts.refAudio)
          ...
          : 0;
        if (!prompt) {
          throw new CliError("minimax-h3-max requires a prompt.", EXIT.USAGE);
        }
        ...
        const referenceCount = imageCount + videoCount + audioCount;
        if (imageCount > 9) {
          throw new CliError(
            "minimax-h3-max accepts at most 9 --ref images.",
            EXIT.USAGE,
          );
        }
        if (videoCount > 3) {
          throw new CliError(
            "minimax-h3-max accepts at most 3 --ref-video clips.",
            EXIT.USAGE,
          );
        }
        if (audioCount > 3) {
          throw new CliError(
            "minimax-h3-max accepts at most 3 --ref-audio clips.",
            EXIT.USAGE,
          );
        }
        if (referenceCount > 12) {
          throw new CliError(
            "minimax-h3-max accepts at most 12 reference files in total.",
            EXIT.USAGE,
          );
        }
        if (audioCount > 0 && imageCount === 0 && videoCount === 0) {
          throw new CliError(
            "minimax-h3-max --ref-audio needs at least one --ref image or --ref-video.",
            EXIT.USAGE,
          );
        }
        if (referenceCount > 0 && (opts.startImage || opts.endImage)) {
          throw new CliError(
            "minimax-h3-max frame mode and reference mode are separate; drop --start-image/--end-image or drop the references.",
            EXIT.USAGE,
          );
        }
        ...
        const h3MaxAspectRatios = ["21:9", "16:9", "4:3", "1:1", "3:4", "9:16"];
        if (
          opts.ar &&
          !(
            referenceCount > 0
              ? ["adaptive", ...h3MaxAspectRatios]
              : h3MaxAspectRatios
          ).includes(opts.ar)
        ) {
          throw new CliError(
            referenceCount > 0
              ? "minimax-h3-max --ar must be adaptive, 21:9, 16:9, 4:3, 1:1, 3:4, or 9:16."
              : "minimax-h3-max --ar must be 21:9, 16:9, 4:3, 1:1, 3:4, or 9:16. adaptive is reference-mode only.",
            EXIT.USAGE,
          );
        }
      }
      if (opts.model === "wan-3.0") {
        ...
        if (
          opts.ar &&
          !["adaptive", "16:9", "4:3", "1:1", "3:4", "9:16"].includes(opts.ar)
        ) {
          throw new CliError(

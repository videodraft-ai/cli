...
export function registerJobCommands(program: Command): void {
  ...
  program
    ...
    .option("--session <id>", "scope to an AI Studio session (incl. shared)")
  ...
  program
    ...
    .action(async function (this: Command, generationId: string) {
      ...
      if (opts.favorite && opts.unfavorite) {
        throw new CliError("Pass --favorite or --unfavorite, not both.", EXIT.USAGE);
      }
      if (opts.favorite || opts.unfavorite) {
      ...
      emit(ctx.out, result, (o) => {
        note(o, `${g?.id ?? generationId}: ${g?.status ?? "unknown"}${g?.isFavorite ? " ★" : ""}`);

...
export function registerPipelineCommands(program: Command): void {
  program
    .command("create <idea...>")
    .description(
      "Create a storyboard project from an idea (script → assets → storyboard)",
    )
    ...
    .option(
      "--no-asset-images",
      "skip generating visual-asset reference images",
    )
    .option(
      "--shots",
      "also batch-generate every shot image (spends credits per shot)",
    )
    .option(
      "--grid",
      "with --shots: grid mode for stronger cross-shot consistency",
    )
    .option(
      "--script-only",
      "stop at the script (script-stage project, no storyboard)",
    )
    .action(async function (this: Command, ideaWords: string[]) {
      ...
      capture("cli_create", {
        script_only: Boolean(opts.scriptOnly),
        shots: Boolean(opts.shots),
      });
      const tool = opts.scriptOnly
        ? "generate_script_from_idea"
        : "generate_storyboard_from_idea";
      const spin = spinner(
        ctx.out,
        opts.scriptOnly
          ? "Generating script…"
          : "Building project (script → assets → storyboard)…",
      );
      let result: any;
      try {
        result = await ctx.client.callTool(
          ...
          compact({
            ...
            generate_asset_images: opts.scriptOnly
              ? undefined
              : opts.assetImages,
            generate_shot_images: opts.scriptOnly
              ? undefined
              : opts.shots || undefined,
        ...
        if (
          ...
          /timed? ?out|abort/i.test(String(err?.message))
        ) {
          throw new CliError(
      ...
      emit(ctx.out, result, (o) => {
        ...
        if (result?.summary)
          note(o, fmt.dim(o, String(result.summary).slice(0, 400)));
        ...
        if (shotJobs.length > 0) {
          note(
            ...
            fmt.dim(
              o,
              `Shot image jobs: ${shotJobs.join(", ")} — poll with videodraft status <job>`,
            ),
          );
        }
      });
  ...
  program
    ...
    .description(
      "Batch-generate the storyboard shot images (spends credits per shot)",
    )
    ...
    .action(async function (this: Command, projectId: string) {
      ...
      if (opts.estimate) {
        ...
        emit(ctx.out, {
          project_scenes:
            project?.storyboard?.scenes?.length ?? project?.scenes?.length,
      ...
      const jobIds: string[] =
        submitted?.job_ids ?? (submitted?.job_id ? [submitted.job_id] : []);
      ...
      const spin = spinner(
        ctx.out,
        `Generating shot images (${jobIds.length} job(s))…`,
      );
      try {
        const resultMap = await pollGenerationsBatch(ctx.client, jobIds, {
          ...
          onTick: (progress) =>
            spin.update(`Generating shot images — ${progress}`),
        });
          ...
          (o) => {
            note(
              ...
              failed === 0
                ? fmt.green(
                    o,
                    `All ${results.length} job(s) completed — images are on the shot cards.`,
                  )
  ...
  program
    ...
    .description(
      "Produce the storyboard into production — animatic (default) or full_video AI Production",
    )
    .option(
      "--mode <mode>",
      "animatic (slideshow, default) | full_video (one Seedance 2 video per scene)",
    )
    .option(
      "--no-auto-videos",
    ...
    .action(async function (this: Command, projectId: string) {
      ...
      try {
        result = await ctx.client.callTool(
          ...
          compact({
            ...
            shot_duration: opts.shotDuration
      ...
      const pendingVideos: string[] =
        result?.pending_scene_video_generations ??
        result?.pending_generation_ids ??
        [];
      emit(ctx.out, result, (o) => {
        if (result?.status === "generating_shot_images") {
          note(
            ...
            fmt.yellow(
              o,
              "Shot images are still generating — poll the job ids below, then re-run produce.",
            ),
          );
          ...
          note(
            ...
            fmt.yellow(
              o,
              result?.summary ?? "Some scene videos were not submitted.",
            ),
          );
          const unresolvedHint = seedanceUnresolvedSubmissionHint(result);
          ...
          note(
            o,
            fmt.green(o, `Producing ${pendingVideos.length} scene video(s).`),
          );
          note(
            o,
            fmt.dim(
              o,
              "Poll: videodraft generations  → then: videodraft finalize " +
                projectId +
                " → videodraft export " +
                projectId,
            ),
          ...
          note(
            o,
            fmt.green(o, "Produced. Next: videodraft export " + projectId),
          );
        }
      });
  ...
  program
    ...
    .description(
      "Swap completed full_video scene videos into the timeline (after produce --mode full_video)",
    )
    .action(async function (this: Command, projectId: string) {
      const ctx = buildContext(this);
      const result: any = await ctx.client.callTool("finalize_scene_videos", {
        project_id: projectId,
      });
      emit(ctx.out, result, (o) => {
        note(
        ...
        if ((result?.pending_remaining ?? result?.still_pending ?? 0) > 0) {
          note(
            ...
            fmt.dim(
              o,
              "Some clips are still rendering — re-run finalize later (it's idempotent).",
            ),
          );
        }
      });
  ...
  program
    ...
    .description(
      "Place a clip/image onto a storyboard shot (e.g. a generated video into the timeline)",
    )
    .requiredOption("--scene <n>", "0-based scene index")
    .requiredOption("--shot <n>", "0-based shot index")
    .requiredOption(
      "--media <url|file>",
      "media to attach (local files are uploaded)",
    )
    .option("--type <type>", "image | video (default image)")
    .option(
      "--duration <seconds>",
      "clip duration in seconds (recommended for video)",
    )
    ...
    .action(async function (this: Command, projectId: string) {
      ...
      emit(ctx.out, result, (o) =>
        note(
          o,
          fmt.green(o, `Attached to scene ${opts.scene}, shot ${opts.shot}.`),
        ),
      );
    });
    ...
    .action(async function (this: Command, projectId: string) {
      ...
      if (opts.wait === false) {
        emit(ctx.out, started, (o) => {
          ...
          note(
            ...
            fmt.dim(
              o,
              `Check with: videodraft export-status ${exportId ?? projectId}`,
            ),
          );
        });
        return;
      ...
      try {
        ...
        if (result.status !== "finished" || !result.videoUrl) {
          // Print the tool's own reason. "Export failed." on every non-finished
          // outcome is what hid "you never started one" and "it finished but the
          // file could not be matched" behind the same three words.
          const reason =
            result.payload?.message ||
            result.payload?.error_message ||
            (result.status === "not_started"
              ? "No export was started for this project."
              : "Export failed.");
          emit(ctx.out, result.payload, (o) => note(o, fmt.red(o, reason)));
          process.exitCode = 1;
          return;
        }
  ...
  program
    ...
    .description(
      "Check a video export — pass the export_id (or --project to pass a project id)",
    )
    .option(
      "--project",
      "treat <id> as a project id (checks its latest export)",
    )
    .option(
      "--wait <seconds>",
      "block up to N seconds (max 240), re-polling until terminal",
    )
  ...
  program
    ...
    .description(
      "Generate advisory per-shot motion/video prompts for a project",
    )
    .option("--ar <ratio>", "aspect ratio")
    .option("--instructions <text>", "authoring instructions")
    .option(
      "--has-voiceover",
      "tell the generator the project has a voiceover track",
    )
    .option(
      "--video-audio",
      "tell the generator that generated videos may include native audio",
    )

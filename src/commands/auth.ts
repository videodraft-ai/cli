...
export function registerAuthCommands(program: Command): void {
  program
    .command("login")
    .description(
      "Authenticate with VideoDraft (browser login; --token for a PAT)",
    )
    .option(
      "--token <vd_mcp_token>",
      "use a personal access token from /mcp-keys instead of the browser",
    )
    ...
    .action(async function (this: Command) {
      ...
      const baseUrl = (
        globals.baseUrl ??
        process.env.VIDEODRAFT_BASE_URL ??
        DEFAULT_BASE_URL
      ).replace(/\/$/, "");
      ...
      if (pat) {
        if (!pat.startsWith("vd_mcp_")) {
          throw new CliError(
            "That does not look like a VideoDraft token (expected vd_mcp_...).",
            EXIT.USAGE,
          );
        }
        ...
        emit(ctxOut, { ok: true, method: "pat", user: me }, (o) => {
          note(
            ...
            fmt.green(
              o,
              `Logged in as ${me?.email ?? me?.user_id ?? "user"} (PAT, ${baseUrl})`,
            ),
          );
        });
        return;
      ...
      if (!known) {
        note(
          ...
          fmt.dim(
            ctxOut,
            `OAuth client ${clientId} not registered on ${baseUrl} — registering one...`,
          ),
        );
      ...
      try {
        ...
        emit(ctxOut, { ok: true, method: "oauth", user: me }, (o) => {
          note(
            o,
            fmt.green(o, `Logged in as ${me?.email ?? "user"} (${baseUrl})`),
          );
          note(
            o,
            fmt.dim(
              o,
              "Tokens stored in " +
                `${process.env.VIDEODRAFT_CONFIG_DIR ?? "~/.config/videodraft"}/config.json`,
            ),
          );
        });
      } catch (err) {
  ...
  program
    .command("logout")
    .description(
      "Revoke the current OAuth grant (best-effort) and clear stored credentials",
    )
    .action(async function (this: Command) {
      const globals = this.optsWithGlobals<{
        profile?: string;
      ...
      if (!profile) {
        emit(ctxOut, { ok: true, message: "No stored credentials." }, (o) =>
          note(o, "No stored credentials."),
        );
        return;
      }
      if (profile.auth_kind === "oauth") {
        ...
        if (profile.refresh_token)
          await revokeToken(profile.base_url, profile.refresh_token);
      }
      updateConfig((config) => {
      ...
      emit(ctxOut, { ok: true }, (o) => {
        ...
        if (profile.auth_kind === "pat") {
          note(
            ...
            fmt.dim(
              o,
              "PATs are revoked from the web app: https://app.videodraft.ai/mcp-keys",
            ),
          );
        }
      });
  ...
  const el = program
    .command("elevenlabs")
    .description(
      "Manage your ElevenLabs API key (BYOK) — use your cloned voices",
    );
  ...
  el.command("status")
    ...
    .action(async function (this: Command) {
      const ctx = buildContext(this);
      const st: any = await ctx.client.restRequest(
        "GET",
        "/api/elevenlabs-key",
      );
      emit(ctx.out, st, (o) => {

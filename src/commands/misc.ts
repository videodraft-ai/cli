    ...
    .action(async function (this: Command, key: string, value: string) {
      ...
      if (key === "telemetry") {
        ...
        updateConfig((c) => {
          ...
          if (!profile)
            throw new CliError(
              "No profile yet — run `videodraft login` first.",
              EXIT.USAGE,
            );
        ...
        throw new CliError(
          `Unsupported key "${key}". Supported: telemetry, base_url.`,
          EXIT.USAGE,
        );
      }
      ...
      const cmd =
        process.platform === "darwin"
          ...
          : process.platform === "win32"
            ...
            : "xdg-open";
      try {
        const child = spawn(cmd, [DOCS_URL], {
          stdio: "ignore",
          detached: true,

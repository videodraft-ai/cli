...
export function registerToolCommands(program: Command): void {
  const tools = program
    .command("tools")
    .description("Inspect the full MCP tool catalog");
  ...
  tools
    ...
    .action(async function (this: Command) {
      ...
      const catalog: any = await ctx.client.callTool(
        "get_tool_catalog",
        // The catalog also carries the full agent guidance text; this
        // listing only prints the tool table, so skip the ~25 KB payload.
        compact({ lane: opts.lane, category: opts.category, guidance: "none" }),
      );
  ...
  program
    ...
    .option(
      "--arg <key=value>",
      "single argument (repeatable; values JSON-coerced)",
      collect,
      [],
    )
    ...
    .action(async function (this: Command, toolName: string) {
      ...
      if (opts.args) {
        ...
        try {
          ...
          throw new CliError(
            `--args is not valid JSON: ${err?.message}`,
            EXIT.USAGE,
          );
        }
        if (
